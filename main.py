...
PROVIDERS = {
    "openai": {"name": "OpenAI", "base_url": "https://api.openai.com/v1", "type": "openai_compat", "models": ["gpt-4o", "gpt-4o-mini", "gpt-4-turbo", "o1", "o1-mini"]},
    "anthropic": {"name": "Anthropic", "base_url": "https://api.anthropic.com/v1", "type": "anthropic", "models": ["claude-opus-4-7", "claude-sonnet-4-6", "claude-haiku-4-5-20251001"]},
    "google": {"name": "Google Gemini", "base_url": "https://generativelanguage.googleapis.com/v1beta", "type": "google", "models": ["gemini-2.5-pro", "gemini-2.0-flash", "gemini-1.5-pro", "gemini-1.5-flash"]},
    "groq": {"name": "Groq", "base_url": "https://api.groq.com/openai/v1", "type": "openai_compat", "models": ["llama-3.3-70b-versatile", "llama-3.1-70b-versatile", "gemma2-9b-it", "mixtral-8x7b-32768"]},
    "mistral": {"name": "Mistral", "base_url": "https://api.mistral.ai/v1", "type": "openai_compat", "models": ["mistral-large-latest", "mistral-small-latest", "codestral-latest"]},
    "xai": {"name": "xAI Grok", "base_url": "https://api.x.ai/v1", "type": "openai_compat", "models": ["grok-2-latest", "grok-2-mini"]},
    "openrouter": {"name": "OpenRouter", "base_url": "https://openrouter.ai/api/v1", "type": "openai_compat", "models": ["openai/gpt-4o", "anthropic/claude-3.5-sonnet", "meta-llama/llama-3.3-70b-instruct", "google/gemini-2.0-flash-001"]},
}
...
def _default_config() -> dict:
    return {"keys": {}, "key_storage": "keyring", "local_model": "gemma3:4b", "ollama_url": "http://localhost:11434", "dev_mode": False}
...
def save_config(cfg: dict):
    cfg = dict(cfg)
    cfg["keys"] = {}
    cfg["key_storage"] = "keyring"
    with open(CONFIG_FILE, "w", encoding="utf-8") as f:
        json.dump(cfg, f, indent=2)


def _keyring_name(provider_id: str) -> str:
    return f"api_key:{provider_id}"


def get_api_key(provider_id: str, cfg: Optional[dict] = None) -> Optional[str]:
    try:
        key = keyring.get_password(KEYRING_SERVICE, _keyring_name(provider_id))
        if key:
            return key
    except Exception:
        logger.exception("Keyring read failed for %s", provider_id)
    # Migration fallback only: read old plaintext config if it exists.
    cfg = cfg or load_config()
    return cfg.get("keys", {}).get(provider_id)


def set_api_key(provider_id: str, value: str):
    try:
        keyring.set_password(KEYRING_SERVICE, _keyring_name(provider_id), value)
    except Exception as e:
        logger.exception("Keyring write failed for %s", provider_id)
        raise HTTPException(status_code=500, detail=f"Could not save API key to OS keyring: {e}")


def delete_api_key(provider_id: str):
    try:
        keyring.delete_password(KEYRING_SERVICE, _keyring_name(provider_id))
    except keyring.errors.PasswordDeleteError:
        pass
    except Exception:
        logger.exception("Keyring delete failed for %s", provider_id)


def migrate_plaintext_keys_to_keyring():
    cfg = load_config()
    old_keys = cfg.get("keys", {})
    if not old_keys:
        return
    migrated = []
    for provider_id, key in old_keys.items():
        if provider_id in PROVIDERS and key:
            try:
                set_api_key(provider_id, key)
                migrated.append(provider_id)
            except HTTPException:
                logger.warning("Could not migrate key for %s", provider_id)
    cfg["keys"] = {}
    cfg["key_storage"] = "keyring"
    save_config(cfg)
    if migrated:
        logger.info("Migrated plaintext API keys to OS keyring: %s", migrated)
...
async def lifespan(app: FastAPI):
    ...
    migrate_plaintext_keys_to_keyring()
...
async def update_settings(req: SettingsUpdate):
    ...
    save_config(cfg)
    logger.info("Settings updated. API keys stored in OS keyring.")
    return {"status": "saved"}
...
async def check_updates():
    try:
        async with httpx.AsyncClient(timeout=8) as client:
            ...
            return {"current": APP_VERSION, "latest": latest, "update_available": latest != APP_VERSION, "changelog": pf.get("changelog", ""), "download": pf.get("download", "")}
    except Exception:
...
async def optimize_prompt(req: OptimizeRequest):
    ...
    try:
        async with httpx.AsyncClient(timeout=90) as client:
            r = await client.post(f"{base}/api/generate", json={"model": model, "prompt": OPTIMIZE_SYSTEM + req.prompt, "stream": False, "options": {"temperature": 0.25, "num_predict": 512}})
            r.raise_for_status()
            data = r.json()
...
async def send_to_ai(req: SendRequest):
    ...
    api_key = get_api_key(req.provider, cfg)
    if not api_key:
    ...
    try:
        async with httpx.AsyncClient(timeout=120) as client:
            if provider["type"] == "openai_compat":
                r = await client.post(f"{provider['base_url']}/chat/completions", headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}, json={"model": req.model, "messages": [{"role": "user", "content": final_prompt}]})
                data = r.json()
                if r.status_code >= 400 or "error" in data:
                    raise HTTPException(status_code=400, detail=data.get("error", {}).get("message", f"API error {r.status_code}"))
                return {"response": data["choices"][0]["message"]["content"], "usage": data.get("usage", {}), "provider": req.provider, "model": req.model}
            if provider["type"] == "anthropic":
                r = await client.post(f"{provider['base_url']}/messages", headers={"x-api-key": api_key, "anthropic-version": "2023-06-01", "Content-Type": "application/json"}, json={"model": req.model, "max_tokens": 4096, "messages": [{"role": "user", "content": final_prompt}]})
                data = r.json()
                if r.status_code >= 400 or "error" in data:
                    raise HTTPException(status_code=400, detail=data.get("error", {}).get("message", f"API error {r.status_code}"))
                usage = data.get("usage", {})
                return {"response": data["content"][0]["text"], "usage": {"input_tokens": usage.get("input_tokens"), "output_tokens": usage.get("output_tokens")}, "provider": req.provider, "model": req.model}
            if provider["type"] == "google":
                r = await client.post(f"{provider['base_url']}/models/{req.model}:generateContent", params={"key": api_key}, json={"contents": [{"parts": [{"text": final_prompt}]}]})
                data = r.json()
                if r.status_code >= 400 or "error" in data:
                    raise HTTPException(status_code=400, detail=data.get("error", {}).get("message", f"API error {r.status_code}"))
                meta = data.get("usageMetadata", {})
                return {"response": data["candidates"][0]["content"]["parts"][0]["text"], "usage": {"input_tokens": meta.get("promptTokenCount"), "output_tokens": meta.get("candidatesTokenCount")}, "provider": req.provider, "model": req.model}
